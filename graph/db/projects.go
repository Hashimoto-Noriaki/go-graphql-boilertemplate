...
var ProjectWhere = struct {
	...
	Name:        whereHelpernull_String{field: "\"projects\".\"name\""},
...
var (
	projectAllColumns            = []string{"id", "name", "description"}
	projectColumnsWithoutDefault = []string{}
	projectColumnsWithDefault    = []string{"id", "name", "description"}
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(projectPrimaryKeyColumns) == 0 {
				return errors.New("db: unable to upsert projects, could not build conflict column list")
			}
